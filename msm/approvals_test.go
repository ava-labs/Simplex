...
func TestApprovalStoreHandleApproval(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			// Verifies that the store keys on (NodeID, PChainHeight, AuxInfoSeqDigest):
			...
			validators: 3,
			approvals: []approvalAndTimestamp{
				{ValidatorSetApproval{NodeID: makeNodeID(1), PChainHeight: 7, AuxInfoSeqDigest: [32]byte{0xAA}, Signature: signApproval(7, [32]byte{0xAA})}, 100},
				{ValidatorSetApproval{NodeID: makeNodeID(1), PChainHeight: 7, AuxInfoSeqDigest: [32]byte{0xBB}, Signature: signApproval(7, [32]byte{0xBB})}, 100},
			},
			verify: func(t *testing.T, as *ApprovalStore, sent []approvalAndTimestamp) {
				got := as.Approvals()
				require.Len(t, got, 2)
