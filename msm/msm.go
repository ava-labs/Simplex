...
// InitializeApprovalStore initializes the approval store for the given validator set
// if it is not already initialized for it.
func (sm *StateMachine) InitializeApprovalStore(validatorSet NodeBLSMappings) {
	sm.maybeInitializeApprovalStore(validatorSet)
}

// Approvals returns the approvals accumulated in the approval store,
// or nil if the store has not been initialized.
func (sm *StateMachine) Approvals() ValidatorSetApprovals {
	sm.lock.RLock()
	...
	return sm.approvalStore.Approvals()
}
