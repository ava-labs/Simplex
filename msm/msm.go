...
// AuxiliaryInfoGenerator generates the AuxiliaryInfo for the next block in case the MSM is collecting approvals.
// The AuxiliaryInfo is opaque information that is included in the block metadata
// and is contributed by the block builder.
type AuxiliaryInfoGenerator interface {
	// GenerateAuxiliaryInfo generates the AuxiliaryInfo for the next block and returns it along with the AppID.
	// The AppID is used to identify the type of the AuxiliaryInfo and it exists for the purpose of backwards compatibility.
	// The input is the previous AuxiliaryInfo, if applicable, which can be used to determine if the current node has already contributed
	// its own AuxiliaryInfo for the current epoch transition.
	GenerateAuxiliaryInfo([]byte) ([]byte, AppID)
}

// AuxiliaryInfoVerifier verifies the AuxiliaryInfo included in the block metadata when we are collecting approvals.
type AuxiliaryInfoVerifier interface {
	// VerifyAuxiliaryInfo verifies the given new AuxiliaryInfo included in the block metadata.
	// The given prevAuxInfos are the AuxiliaryInfo from previous blocks in the current epoch transition,
	// which can be used for verification.
	VerifyAuxiliaryInfo(auxInfo *AuxiliaryInfo, prevAuxInfos []AuxiliaryInfo) error
}
...
type Config struct {
	// AuxiliaryInfoVerifier verifies the AuxiliaryInfo included in the block metadata when we are collecting approvals for an epoch transition.
	AuxiliaryInfoVerifier AuxiliaryInfoVerifier
	// AuxiliaryInfoGenerator generates the AuxiliaryInfo for the next block when we are collecting approvals for an epoch transition.
...
func NewStateMachine(config Config) *StateMachine {
	...
	out.verifiers = []verifier{
		...
		&icmEpochInfoVerifier{
			computeICMEpoch: config.ComputeICMEpoch,
			getUpdates:      config.GetUpgrades,
		},
...
func (sm *StateMachine) buildBlockNormalOp(ctx context.Context, parentBlock StateMachineBlock, simplexMetadata simplex.ProtocolMetadata, blacklist []byte) (*StateMachineBlock, error) {
	...
	case blockBuildingDecisionTransitionEpoch:
		...
		return sm.wrapBlock(parentBlock, nil, newSimplexEpochInfo, pChainHeight, simplexMetadata, blacklist, nil), nil
...
func (sm *StateMachine) buildBlockAndMaybeTransitionEpoch(
	...
	return sm.wrapBlock(parentBlock, childBlock, newSimplexEpochInfo, pChainHeight, simplexMetadata, blacklist, nil), nil
}
...
func (sm *StateMachine) buildBlockImpatiently(ctx context.Context, parentBlock StateMachineBlock, simplexMetadata simplex.ProtocolMetadata, blacklist []byte, simplexEpochInfo SimplexEpochInfo, pChainHeight uint64) (*StateMachineBlock, error) {
	...
	newAuxInfo, err := sm.generateAuxInfoForNextBlock(parentBlock, simplexMetadata, simplexEpochInfo)
	if err != nil {
		return nil, err
	}

	return sm.wrapBlock(parentBlock, childBlock, simplexEpochInfo, pChainHeight, simplexMetadata, blacklist, newAuxInfo), nil
}

func (sm *StateMachine) generateAuxInfoForNextBlock(parentBlock StateMachineBlock, simplexMetadata simplex.ProtocolMetadata, simplexEpochInfo SimplexEpochInfo) (*AuxiliaryInfo, error) {
	// The zero block shouldn't have auxiliary info.
	if parentBlock.Metadata.SimplexEpochInfo.EpochNumber == 0 {
		return nil, nil
	}

	// The sealing block also shouldn't have auxiliary info, as we have decided that we can seal the epoch
	// based on the auxiliary info of the previous block.
	if simplexEpochInfo.BlockValidationDescriptor != nil {
		return nil, nil
	}

	parentSeq := simplexMetadata.Seq - 1
	prevAuxInfoSeq, prevAuxInfo, err := sm.retrievePreviousAuxiliaryInfo(parentBlock, parentSeq)
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve previous AuxiliaryInfo: %w", err)
	}

	auxInfo, appID := sm.AuxiliaryInfoGenerator.GenerateAuxiliaryInfo(prevAuxInfo)

	newAuxInfo := &AuxiliaryInfo{
		PrevAuxInfoSeq: prevAuxInfoSeq,
		ApplicationID:  appID,
		Info:           auxInfo,
	}
	return newAuxInfo, nil
}

func (sm *StateMachine) retrievePreviousAuxiliaryInfo(parentBlock StateMachineBlock, parentSeq uint64) (uint64, []byte, error) {
	prevAuxInfo := parentBlock.Metadata.AuxiliaryInfo

	if prevAuxInfo == nil || prevAuxInfo.IsZero() {
		return 0, nil, nil
	}

	if len(prevAuxInfo.Info) > 0 {
		return parentSeq, prevAuxInfo.Info, nil
	}

	if prevAuxInfo.PrevAuxInfoSeq > 0 {
		block, _, err := sm.GetBlock(prevAuxInfo.PrevAuxInfoSeq, [32]byte{})
		if err != nil {
			return 0, nil, fmt.Errorf("failed to retrieve block for previous AuxiliaryInfo sequence %d: %w", prevAuxInfo.PrevAuxInfoSeq, err)
		}
		if block.Metadata.AuxiliaryInfo == nil {
			return 0, nil, fmt.Errorf("block for previous AuxiliaryInfo sequence %d has no AuxiliaryInfo in metadata", prevAuxInfo.PrevAuxInfoSeq)
		}
		return prevAuxInfo.PrevAuxInfoSeq, block.Metadata.AuxiliaryInfo.Info, nil
	}

	// Else, prevAuxInfo.PrevAuxInfoSeq is 0 and prevAuxInfo.Info is empty.
	return 0, nil, nil
}
...
func (sm *StateMachine) wrapBlock(parentBlock StateMachineBlock, childBlock VMBlock, newSimplexEpochInfo SimplexEpochInfo, pChainHeight uint64, simplexMetadata simplex.ProtocolMetadata, blacklist []byte, auxInfo *AuxiliaryInfo) *StateMachineBlock {
...
func (sm *StateMachine) buildBlockEpochSealed(ctx context.Context, parentBlock StateMachineBlock, simplexMetadata simplex.ProtocolMetadata, blacklist []byte) (*StateMachineBlock, error) {
	...
	return sm.wrapBlock(parentBlock, childBlock, newSimplexEpochInfo, parentBlock.Metadata.PChainHeight, simplexMetadata, blacklist, nil), nil
}
