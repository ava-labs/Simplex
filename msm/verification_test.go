...
func TestPrevSealingBlockHashVerifier(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			err: fmt.Sprintf("expected prev sealing inner block hash to be %x but got %x", prevSealingBlockHash, [32]byte{9, 9, 9}),
		},
		{
		...
		},
		{
			...
			err: fmt.Sprintf("expected prev sealing inner block hash of a non sealing inner block to be empty but got %x", [32]byte{}),
		},
		{
...
func TestNextPChainReferenceHeightVerifier(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			err:           "expected P-chain reference height to be 0 but got 0",
		},
		{
...
func (bs testBlockStore) getBlock(opts RetrievingOpts) (StateMachineBlock, *simplex.Finalization, error) {
	...
	if !ok {
		return StateMachineBlock{}, nil, fmt.Errorf("block %d not found", opts.Height)
	}
	return blk, nil, nil
}
