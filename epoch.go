...
func (e *Epoch) maybeCollectFinalization(round *Round) error {
	...
	for _, finalizationsWithTheSameDigest := range finalizationsByMD {
		if e.SignatureAggregator.IsQuorum(nodeIDsFromFinalizeVotes(finalizationsWithTheSameDigest)) {
...
func (e *Epoch) maybeAssembleEmptyNotarization() error {
	...
	popularEmptyVote, signatures, found := findMostPopularEmptyVote(emptyVotes.votes)
	if !found {
		e.Logger.Debug("Could not find empty vote with a quorum or more votes", zap.Uint64("round", e.round))
		return nil
	}

	if !e.SignatureAggregator.IsQuorum(nodeIDsFromSignatures(signatures)) {
		e.Logger.Debug("Not enough signatures to form an empty notarization", zap.Uint64("round", e.round), zap.Int("signatures", len(signatures)))
		return nil
	}
...
func findMostPopularEmptyVote(votes map[string]*EmptyVote) (ToBeSignedEmptyVote, []Signature, bool) {
	atLeastHalf := len(votes)/2 + 1
