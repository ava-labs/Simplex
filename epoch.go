...
const (
	...
	DefaultFinalizationRebroadcastTimeout = 5 * time.Second
...
type Epoch struct {
	...
	oldestUnFinalizedNotarization  NotarizationTime
...
func (e *Epoch) AdvanceTime(t time.Time) {
	...
	e.checkForUnFinalizedNotarizedBlocks(t)
}
...
func (e *Epoch) init() error {
	...
	e.initOldestUnFinalizedNotarization()
...
func (e *Epoch) initOldestUnFinalizedNotarization() {
	e.oldestUnFinalizedNotarization = NewNotarizationTime(
		e.FinalizationRebroadcastTimeout,
		e.haveUnFinalizedNotarizedRound,
		func() {
...
func (e *Epoch) checkForUnFinalizedNotarizedBlocks(now time.Time) {
	e.lock.Lock()
	defer e.lock.Unlock()

	e.oldestUnFinalizedNotarization.CheckForUnFinalizedNotarizedBlocks(now)
}

func (e *Epoch) haveUnFinalizedNotarizedRound() (uint64, bool) {
	for _, round := range e.rounds {
		if round.notarization != nil && round.finalization == nil {
			return round.num, true
		}
	}
