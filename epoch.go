...
func (e *Epoch) verifyProposalMetadataAndBlacklist(block Block) bool {
	...
	if err := verifyProposedBlacklist(prevBlacklist, block.Blacklist(), len(e.nodes), e.round); err != nil {
...
func (e *Epoch) buildBlock() {
	...
	updates := prevBlacklist.ComputeBlacklistUpdates(uint16(len(e.nodes)), e.timedOutRounds, e.redeemedRounds)
	// 3) Apply the updates to the blacklist.
	nextBlacklist := prevBlacklist
	nextBlacklist.Update(updates, metadata.Round)
	...
	task := e.createBlockBuildingTask(metadata, nextBlacklist)
...
func (e *Epoch) monitorProgress(round uint64) {
	...
	blockShouldBeBuiltNotification := func() {
		...
		// If so, we just jump to the next round.
		metadata := e.metadata()
