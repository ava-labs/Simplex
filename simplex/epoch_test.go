...
type failingBlockBuilder struct {
	buildAttempted chan struct{}
}

func (f *failingBlockBuilder) BuildBlock(context.Context, ProtocolMetadata, Blacklist) (VerifiedBlock, bool) {
	select {
	case f.buildAttempted <- struct{}{}:
	default:
	}
	return nil, false
}

func (f *failingBlockBuilder) WaitForPendingBlock(ctx context.Context) {
	<-ctx.Done()
}
...
func TestEpochWarnsOnBlockBuildFailure(t *testing.T) {
	nodes := []NodeID{{1}, {2}, {3}, {4}}
	bb := &failingBlockBuilder{buildAttempted: make(chan struct{}, 1)}
	conf, _, _ := testutil.DefaultTestNodeEpochConfig(t, nodes[0], testutil.NewNoopComm(nodes), bb)

	warned := make(chan struct{}, 1)
	conf.Logger.(*testutil.TestLogger).Intercept(func(entry zapcore.Entry) error {
		if entry.Level == zapcore.WarnLevel && entry.Message == "Failed building block" {
			select {
			case warned <- struct{}{}:
			default:
			}
	...
	select {
	case <-bb.buildAttempted:
	case <-time.After(5 * time.Second):
		require.Fail(t, "timed out waiting for block build attempt")
	}

	select {
	case <-warned:
	case <-time.After(2 * time.Second):
		require.Fail(t, "build failure was not logged at warn")
	}
}
