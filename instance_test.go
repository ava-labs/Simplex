...
func TestInstanceNonValidatorEvictsFinalizedBlocks(t *testing.T) {
	...
	// Single-validator set that never includes the second node, so it runs as a non-validator.
	validatorSetsAtHeight := map[uint64]metadata.NodeBLSMappings{
		basePChainHeight: {
			{NodeID: id, BLSKey: []byte{0xaa}, Weight: 1},
		},
	...
	// The validator runs alone first and commits a few blocks.
	net.register(validatorNodeID, validatorInstance)
	require.NoError(t, validatorInstance.Start(t.Context()))
	t.Cleanup(validatorInstance.Stop)

	waitForNumBlocks(t, storage, 5)

	// The non-validator joins and replicates the chain.
	net.register(nonValidatorNodeID, nonValidatorInstance)
	require.NoError(t, nonValidatorInstance.Start(t.Context()))
	t.Cleanup(nonValidatorInstance.Stop)

	waitForNumBlocks(t, storage2, storage.NumBlocks())

	// Stop the non-validator so no block is mid-verification while we assert.
	nonValidatorInstance.Stop()

	// Every replicated block below the tip is finalized in storage, so retrieving it
	// by digest must return its finalization.
	numBlocks := storage2.NumBlocks()
	for seq := uint64(1); seq < numBlocks; seq++ {
		...
		require.NotNilf(t, finalization, "finalized block at seq %d served without its finalization", seq)
	}
}
