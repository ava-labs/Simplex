...
func TestNonValidator_BecomesValidator(t *testing.T) {
	...
	pChain.setValidatorSetAt(10, []metadata.NodeBLSMapping{validator, upcomingValidator})
...
func TestValidator_ValidatorSetNotChanged(t *testing.T) {
	...
	firstBlock := chain.acceptNewBlock()
	...
	secondBlock := chain.acceptNewBlock()
...
func TestValidator_ValidatorSetDecreased(t *testing.T) {
	...
	require.Contains(t, sealing.SealingBlockInfo().ValidatorSet.NodeIDs(), common.NodeID(validator.NodeID[:]))
	require.NotContains(t, sealing.SealingBlockInfo().ValidatorSet.NodeIDs(), common.NodeID(leavingValidator.NodeID[:]))
}
