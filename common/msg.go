...
type AuxiliaryInfo struct {
	Version VersionID // version code used to generate this auxiliary info
	Data    []byte    // the generated bytes
}

type AvaGoNodeID [20]byte

type ValidatorSetApproval struct {
	NodeID        AvaGoNodeID `canoto:"fixed bytes,1"`
...
func (vsa ValidatorSetApprovals) UniqueByNodeID() ValidatorSetApprovals {
	seen := make(map[AvaGoNodeID]struct{})
	result := make(ValidatorSetApprovals, 0, len(vsa))
	for _, v := range vsa {
