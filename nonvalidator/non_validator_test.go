...
// same leader as the validators. LeaderForRound is order sensitive and validators sort their set
// in Epoch.init, so keeping the set in the order it arrived makes handleBlock drop every live
// proposal as not being from the leader of that round.
